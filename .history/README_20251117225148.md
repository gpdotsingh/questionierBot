...
from splitter import split_query_simple; \
print(json.dumps(split_query_simple('Show decline in total donation amount; then for the worst 3 states list top donors'), indent=2))"



uv run python -c 'import sys,json; sys.path.insert(0,"src"); from qapipeline.splitter import QuestionSplitter; qs=QuestionSplitter(); plan=qs.plan("Find donors with EngagementScore ≥ 85 and EventParticipation = Yes; then produce three sibling outputs: (a) state distribution, (b) top 10 cities, (c) top 20 donors by TotalAmountDonated."); print(json.dumps({"original_question":plan.original_question,"used_llm":plan.used_llm,"steps":[{"id":s.id,"text":s.text} for s in plan.ordered_steps]}, indent=2))'


python -c "import json,sys; sys.path.insert(0,'src'); \from qapipeline.splitter import QuestionSplitter; \print(json.dumps(split_query_simple('Find donors with EngagementScore ≥ 85 and EventParticipation = Yes; then produce three sibling outputs: (a) state distribution, (b) top 10 cities, (c) top 20 donors by TotalAmountDonated.'), indent=2))"
