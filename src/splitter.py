...
class _LLMRouter:
    ...
    def ask_json(self, prompt: str) -> Optional[dict]:
        ...
        try:
            if self._openai:
                # Try JSON mode if supported; else best-effort
...
def _vector_terms(query: str, top_k: int = 8, faiss_dir: str = "faiss_store") -> List[str]:
    ...
    # Normalize: two-letter tokens -> UPPER (state codes), others -> lower
    out, seen = [], set()
    for t in bag:
...
def _number_from_llm_dict(llm_dict: dict) -> Dict[str, Any]:
    """
    LLM returns a dict like:
      {"Q1":{"text":"...","children":[{"Q2":{...}}, {"Q3":{...}}]}, "Q4":{...}}
    If it returns a node schema like {"nodes":[{"text":"..","children":[...]}]}, number depth-first.
    """
    ...
    # Depth-first numbering over list of roots
    qid = 0
...
def split_query_simple(user_query: str) -> Dict[str, Any]:
    """
    1) Loads env and metadata
    2) Prompts LLM with metadata to split; if fails, adds vector hints and retries
    3) If still no luck (or no LLM), uses rule-based fallback
    4) Always returns final JSON in:
       { "Q1": {"text":"...","children":[{"Q2":{...}}]}, "Q4":{...} }
    """
    router = _LLMRouter()
    meta = _load_metadata()
    meta_text = _metadata_text(meta)

    # ---- (A) LLM with metadata only
    if router.provider:
        ...
        # ---- (B) LLM with vector hints fallback
    ...
    # ---- (C) Final fallback: deterministic rule-based
    augmented = user_query
    if not router.provider:
        # no LLM -> try adding retrieval terms so the rule split has more material
